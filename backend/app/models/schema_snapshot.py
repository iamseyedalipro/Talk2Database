...
class SchemaSnapshot(Base, TimestampMixin):
    ...
    # keyed by UI language: {"en": [...], "fa": [...]}. Older rows may hold a
    # bare list (English-only); readers normalize it. A schema change creates a
    # new snapshot row, so the cache self-invalidates.
    suggested_questions_json: Mapped[dict[str, list[str]] | list[str] | None] = mapped_column(
        JSON, nullable=True
    )
