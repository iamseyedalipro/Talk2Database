...
Primarily **structural metadata** is sent to the AI provider: table and column names, data types, nullability, primary keys, foreign keys, and any object/column comments. This comes from `information_schema`/`pg_catalog` introspection (`app/services/schema/introspect.py`) run through the read-only connection. Results of executed queries are returned only to the authenticated user in the panel; they are not sent back to the model.

**Allowed-value grounding.** To stop the model inventing values (e.g. `status = 'paid'` when only `successful`/`failed`/`pending` exist), each column's allowed values are surfaced in the schema block. Enum labels and `CHECK`-constraint values are read from the catalog with **no row reads**. In addition, when `SCHEMA_SAMPLE_VALUES=true` (the default), plain text columns are probed with a bounded `SELECT DISTINCT` (`app/services/schema/sample.py`): at most `SCHEMA_SAMPLE_MAX_VALUES` distinct values (default 25) over at most `SCHEMA_SAMPLE_SCAN_LIMIT` scanned rows (default 10000); columns exceeding the cap are treated as free-text and left unannotated. This is the one place introspection reads a small set of *distinct column values* and sends them to the provider — set `SCHEMA_SAMPLE_VALUES=false` to keep introspection strictly structure-only.
