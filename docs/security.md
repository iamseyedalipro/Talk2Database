...
**One deliberate, opt-in exception: answer summaries.** When `ANSWER_SUMMARY_ENABLED=true`, the client may call `POST /api/summarize` after a query runs to get a short natural-language answer (e.g. "Your total income for 2025 was $48,210"). That call sends the question, the SQL, and a **capped sample of the result rows** to the AI provider. The sample is truncated server-side — at most `ANSWER_SUMMARY_MAX_ROWS` rows (default 20), `ANSWER_SUMMARY_MAX_COLUMNS` columns (default 15), and `ANSWER_SUMMARY_MAX_CELL_CHARS` characters per cell (default 200) — regardless of what the client submits. The feature is **off by default**; leave it off if your data must never reach the provider.
